...
class RapportRentreeController extends Controller
{
    ...
    public function saveGeometrie(Request $request, Rapport $rapport)
    {
        $validated = $request->validate([
            // Instruments de géométrie (nouveaux champs)
            'regle_plate_total' => 'nullable|integer|min:0',
            'regle_plate_bon_etat' => 'nullable|integer|min:0',
            'equerre_total' => 'nullable|integer|min:0',
            'equerre_bon_etat' => 'nullable|integer|min:0',
            ...
            // Besoins et budget
            'besoins_geometrie' => 'nullable|string',
            'budget_estime_geometrie' => 'nullable|integer|min:0',
            'observations_geometrie' => 'nullable|string'
        ]);

        // Convertir les valeurs null en 0 pour les champs integer NOT NULL
        $fieldsToConvert = [
        ...
        if (isset($validated['budget_estime_geometrie']) && $validated['budget_estime_geometrie'] === '') {
            $validated['budget_estime_geometrie'] = null;
        }
        if (isset($validated['besoins_geometrie']) && trim($validated['besoins_geometrie']) === '') {
            $validated['besoins_geometrie'] = null;
        }
        if (isset($validated['observations_geometrie']) && trim($validated['observations_geometrie']) === '') {
            $validated['observations_geometrie'] = null;
        }

        $rapport->geometrie()->updateOrCreate(
            ['rapport_id' => $rapport->id],
            $validated
        );

        return response()->json(['success' => true, 'message' => 'Matériel de Géométrie sauvegardé']);
    }

    public function saveMesure(Request $request, Rapport $rapport)
    {
        $validated = $request->validate([
            // Instruments de mesure longueur
            'decametres' => 'nullable|integer|min:0',
            'metres_plies' => 'nullable|integer|min:0',
            'centimetres' => 'nullable|integer|min:0',
            'reglets' => 'nullable|integer|min:0',
            
            // Instruments de mesure masse
            'balances_plateaux' => 'nullable|integer|min:0',
            'balances_electroniques' => 'nullable|integer|min:0',
            'poids_masses' => 'nullable|integer|min:0',
            
            // Instruments de mesure capacité
            'recipients_gradues' => 'nullable|integer|min:0',
            'eprouvettes' => 'nullable|integer|min:0',
            'verres_doseurs' => 'nullable|integer|min:0',
            
            // Instruments de mesure temps
            'chronometres' => 'nullable|integer|min:0',
            'horloges_demonstration' => 'nullable|integer|min:0',
            'sabliers' => 'nullable|integer|min:0',
            
            // État général et besoins
            'etat_instruments_mesure' => 'nullable|in:excellent,bon,moyen,mauvais',
            'besoins_mesure' => 'nullable|string',
            'budget_estime_mesure' => 'nullable|integer|min:0',
            'observations_mesure' => 'nullable|string'
        ]);

        $rapport->mesure()->updateOrCreate(
            ['rapport_id' => $rapport->id],
            $validated
        );

        return response()->json(['success' => true, 'message' => 'Matériel de Mesure sauvegardé']);
    }
